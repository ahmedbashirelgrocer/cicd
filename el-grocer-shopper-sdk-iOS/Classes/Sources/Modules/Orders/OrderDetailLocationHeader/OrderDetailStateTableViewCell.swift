...
extension OrderDetailStateTableViewCell {
    ...
    func setOrderData( order : Order? , orderProducts : [Product] , orderItems : [ShoppingBasketItem] ) {
        ...
        self.lblNumberOfItems.text = "(" + ElGrocerUtility.sharedInstance.setNumeralsForLanguage(numeral: "\( ((order?.totalProducts ?? 0) > 0) ? Int((order?.totalProducts ?? 0)):(orderProducts.count)) ") + countLabel + ")"
