...
- `AFURLSessionManagerCustom`
- `AFHTTPSessionManagerCustom`

### Serialization

* `<AFURLRequestSerializationCustom>`
  - `AFHTTPRequestSerializerCustom`
  - `AFJSONRequestSerializer`
  - `AFPropertyListRequestSerializerCustomCustom`
* `<AFURLResponseSerializationCustom>`
  - `AFHTTPResponseSerializerCustom`
  - `AFJSONResponseSerializerCustom`
  - `AFXMLParserResponseSerializerCustom`
  - `AFXMLDocumentResponseSerializer` _(macOS)_
  - `AFPropertyListResponseSerializerCustom`
  - `AFImageResponseSerializerCustom`
  - `AFCompoundResponseSerializerCustom`

### Additional Functionality

- `AFSecurityPolicyCustom`
- `AFNetworkReachabilityManagerCustom.h`
...
AFURLSessionManagerCustom *manager = [[AFURLSessionManagerCustom alloc] initWithSessionConfiguration:configuration];
...
```objective-c
NSMutableURLRequest *request = [[AFHTTPRequestSerializerCustom serializer] multipartFormRequestWithMethod:@"POST" URLString:@"http://example.com/upload" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        [formData appendPartWithFileURL:[NSURL fileURLWithPath:@"file://path/to/image.jpg"] name:@"file" fileName:@"filename.jpg" mimeType:@"image/jpeg" error:nil];
    } error:nil];

AFURLSessionManagerCustom *manager = [[AFURLSessionManagerCustom alloc] initWithSessionConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
...
```objective-c
[[AFHTTPRequestSerializerCustom serializer] requestWithMethod:@"GET" URLString:URLString parameters:parameters error:nil];
```
...
```objective-c
[[AFHTTPRequestSerializerCustom serializer] requestWithMethod:@"POST" URLString:URLString parameters:parameters error:nil];
```
...
```objective-c
[[AFNetworkReachabilityManagerCustom.h sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatusCustom status) {
    NSLog(@"Reachability: %@", AFStringFromNetworkReachabilityStatusCustom(status));
}];

[[AFNetworkReachabilityManagerCustom.h sharedManager] startMonitoring];
```
...
`AFSecurityPolicyCustom` evaluates server trust against pinned X.509 certificates and public keys over secure connections.
...
```objective-c
AFHTTPSessionManagerCustom *manager = [AFHTTPSessionManagerCustom manager];
