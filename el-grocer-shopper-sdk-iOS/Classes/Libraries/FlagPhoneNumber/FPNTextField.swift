...
open class FPNTextField: UITextField, FPNCountryPickerDelegate, FPNDelegate {
    ...
    @objc public var flagSize: CGSize = CGSize(width: 0.1, height: 0.1) {
        didSet {
            layoutSubviews()
        }
    }
    ...
    private func updateLeftView() {
        ...
        let height: CGFloat = max(bounds.size.height, leftViewSize.height)
        let newRect: CGRect = CGRect(x: leftViewFrame.minX, y: 0 , width: width, height: height)
    ...
    internal func fpnDidSelect(country: FPNCountry) {
        ...
        DispatchQueue.main.async { [unowned self] in
            ...
            (self.customDelegate as? FPNTextFieldCustomDelegate)?.fpnDidSelectCountry(name: country.name, dialCode: country.phoneCode, code: country.code.rawValue)
        }
